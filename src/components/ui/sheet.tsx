...
const sheetVariants = cva(
	"fixed z-50 gap-4 bg-background p-6 shadow-[0_8px_10px_-5px_rgba(0,0,0,0.2),_0_16px_24px_2px_rgba(0,0,0,0.14),_0_6px_30px_5px_rgba(0,0,0,0.12)] transition ease-in-out data-[closed]:duration-300 data-[expanded]:duration-500 data-[expanded]:animate-in data-[closed]:animate-out",
	{
		variants: {
			position: {
...
const SheetContent = <T extends ValidComponent = "div">(
	...
	return (
		<SheetPortal position={local.position}>
			...
			>
				{local.children}
				<SheetPrimitive.CloseButton class="absolute right-4 top-4 rounded-full opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary p-1">
					<svg

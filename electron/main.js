...
const socketPath = isWindows
	? `\\\\.\\pipe\\mpv-solidsonic-${pid}`
	: `/tmp/mpv-solidsonic-${pid}.sock`;
...
const createMpv = async (options = {}) => {
	...
	if (!effectiveBinary) {
		throw new Error(
			"MPV binary not found. Please install MPV or specify the path in settings.",
		);
	}
	...
	mpv.on("status", (status) => {
		if (status.property === "playlist-pos") {
			...
			if (status.value !== 0) {
				// MPV auto-advanced to next track in playlist
...
function setupMpvIpcHandlers() {
	...
	// This allows us to easily update the next song without disturbing playback.
	ipcMain.on("mpv:autoNext", async (_, url) => {
		const mpv = getMpvInstance();
		if (!mpv) return;

		try {
			// Remove the finished track at position 0
			await mpv.playlistRemove(0).catch(() => {
				mpv.pause();
			});

			// Append the next-next track for gapless playback
			if (url) {
				await mpv.load(url, "append");
			}
		} catch (err) {
